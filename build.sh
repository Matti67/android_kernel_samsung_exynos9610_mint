...
if [[ ! -z ${BUILD_KERNEL_BRANCH} ]]; then
	...
	if [[ ${BUILD_KERNEL_BRANCH} == "mainline" ]]; then
		...
		LOCALVERSION=' - Mint Beta'
		export LOCALVERSION=' - Mint Beta'

		if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
			FILE_OUTPUT=MintBeta-${GITHUB_RUN_NUMBER}_${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}-${FILE_NAME_SELINUX}_${BUILD_DEVICE_NAME^}_${BUILD_DATE}_CI.zip
		else
			FILE_OUTPUT=MintBeta-${GITHUB_RUN_NUMBER}_${BUILD_ANDROID_PLATFORM}_${FILE_KERNEL_CODE}-${FILE_NAME_SELINUX}-NoRoot_${BUILD_DEVICE_NAME^}_${BUILD_DATE}_CI.zip
		fi
	fi
else
	if [[ ${BUILD_KERNEL_MAGISK} == 'true' ]]; then
